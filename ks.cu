...
int main(int argc, char ** argv)
{
	...
	/* double maxK = 12.0; */
	/* double minK = 0.1; */
	/* /1* for (int i_k = 0; i_k < nk; i_k++) { *1/ */
	/* /1* 	h_k_grid[i_k] = maxK*pow(1-p.ddelta,nk-1-i_k); *1/ */
	/* /1* }; *1/ */
	...
	/* /1* tauchen(p.rrhoz, p.ssigmaz, h_logZ, h_PZ, tauchenwidth); *1/ */
	/* /1* for (int i_z = 0; i_z < nz; i_z++) { *1/ */
	/* /1* 	h_z_grid[i_z] = exp(h_logZ[i_z]); *1/ */
	/* /1* }; *1/ */
	...
	/* /1* markovsimul(SIMULPERIOD,CDF_z.hptr,nz,innov_z.hptr,(nz-1)/2,zind_sim.hptr); *1/ */
	/* /1* markovsimul(SIMULPERIOD,CDF_ssigmax.hptr,nssigmax,innov_ssigmax.hptr,(nssigmax-1)/2,ssigmaxind_sim.hptr); *1/ */
	...
	/* 	/1* save_vec(h_V,"./results/Vgrid.csv");               // in #include "cuda_helpers.h" *1/ */
	/* 	/1* save_vec(h_active,"./results/active.csv");         // in #include "cuda_helpers.h" *1/ */
	/* 	/1* save_vec(h_koptind,"./results/koptind.csv");       // in #include "cuda_helpers.h" *1/ */
	/* 	/1* save_vec(h_kopt,"./results/kopt.csv");             // in #include "cuda_helpers.h" *1/ */
	...
	/* 					) */
	/* 				); */
	/* 		/1* printf("simulate forward done\n"); *1/ */
